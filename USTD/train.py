...
if __name__ == '__main__':
    ...
    # --- wandb: one run per training job. group=dataset, job_type=model, so
    # DiffSTG/USTD/AGCRN/CLCRN runs on the same dataset land in the same
    # dashboard view even though they're launched as separate processes/jobs.
        ...
        for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):  # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
            ...
            for i, data in enumerate(dataset):  # inner loop within one epoch
                ...
                if total_iters % opt.print_freq == 0:   # display images on visdom and save images to
                    ...
                    log_metrics(losses, step=total_iters, prefix='train')
